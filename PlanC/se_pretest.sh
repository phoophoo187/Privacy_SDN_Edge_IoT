...
#@se
#edges_control_plane_information
...
control_interface="wlx000f001491d7"
data_interface="wlx000f00101023"
...
sudo ovs-ofctl add-flow br0 ip,priority=100,in_port=LOCAL,nw_dst=$edge4_control_ip,actions="resubmit(,2)" #e1

sudo ovs-ofctl add-flow br0 arp,priority=100,in_port=LOCAL,nw_dst=$edge5_control_ip,actions="resubmit(,3)" #e2
sudo ovs-ofctl add-flow br0 ip,priority=100,in_port=LOCAL,nw_dst=$edge5_control_ip,actions="resubmit(,3)" #e2

sudo ovs-ofctl add-flow br0 arp,priority=100,in_port=LOCAL,nw_dst=$edge6_control_ip,actions="resubmit(,4)" #e3
sudo ovs-ofctl add-flow br0 ip,priority=100,in_port=LOCAL,nw_dst=$edge6_control_ip,actions="resubmit(,4)" #e3
...
sudo ovs-ofctl add-flow br1 ip,priority=100,in_port=LOCAL,nw_dst=$edge4_data_ip,actions="resubmit(,2)" #e1

sudo ovs-ofctl add-flow br1 arp,priority=100,in_port=LOCAL,nw_dst=$edge5_data_ip,actions="resubmit(,3)" #e2
sudo ovs-ofctl add-flow br1 ip,priority=100,in_port=LOCAL,nw_dst=$edge5_data_ip,actions="resubmit(,3)" #e2

sudo ovs-ofctl add-flow br1 arp,priority=100,in_port=LOCAL,nw_dst=$edge6_data_ip,actions="resubmit(,4)" #e3
sudo ovs-ofctl add-flow br1 ip,priority=100,in_port=LOCAL,nw_dst=$edge6_data_ip,actions="resubmit(,4)" #e3
...
#Table 3 is to rewrite the destination MAC address into edge2 MAC address @ control plane
sudo ovs-ofctl add-flow br0 table=3,actions=mod_dl_dst:$edge2_control_mac,"load:0->OXM_OF_IN_PORT[],resubmit(,5)"
#Table 3 is to rewrite the destination MAC address into edge2 MAC address @ data plane
sudo ovs-ofctl add-flow br1 table=3,actions=mod_dl_dst:$edge2_data_mac,"load:0->OXM_OF_IN_PORT[],resubmit(,5)"
