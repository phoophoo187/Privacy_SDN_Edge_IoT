...
#Receive the incoming control traffic to edge5 from edge2,edge4 and edge6
sudo ovs-ofctl add-flow br0 arp,priority=100,in_port=1,dl_src=$edge2_control_mac,arp_tpa=$edge5_control_ip,actions=LOCAL
sudo ovs-ofctl add-flow br0 arp,priority=100,in_port=1,dl_src=$edge4_control_mac,arp_tpa=$edge5_control_ip,actions=LOCAL
sudo ovs-ofctl add-flow br0 arp,priority=100,in_port=1,dl_src=$edge6_control_mac,arp_tpa=$edge5_control_ip,actions=LOCAL
sudo ovs-ofctl add-flow br0 ip,priority=100,in_port=1,dl_src=$edge2_control_mac,nw_dst=$edge5_control_ip,actions=LOCAL
sudo ovs-ofctl add-flow br0 ip,priority=100,in_port=1,dl_src=$edge4_control_mac,nw_dst=$edge5_control_ip,actions=LOCAL
sudo ovs-ofctl add-flow br0 ip,priority=100,in_port=1,dl_src=$edge6_control_mac,nw_dst=$edge5_control_ip,actions=LOCAL

#Receive the incoming data traffic to edge5 from edge2,edge4 and edge6
sudo ovs-ofctl add-flow br1 arp,priority=100,in_port=2,dl_src=$edge2_data_mac,arp_tpa=$edge5_data_ip,actions=LOCAL
sudo ovs-ofctl add-flow br1 arp,priority=100,in_port=2,dl_src=$edge4_data_mac,arp_tpa=$edge5_data_ip,actions=LOCAL
sudo ovs-ofctl add-flow br1 arp,priority=100,in_port=2,dl_src=$edge6_data_mac,arp_tpa=$edge5_data_ip,actions=LOCAL
sudo ovs-ofctl add-flow br1 ip,priority=100,in_port=2,dl_src=$edge2_data_mac,nw_dst=$edge5_data_ip,actions=LOCAL
sudo ovs-ofctl add-flow br1 ip,priority=100,in_port=2,dl_src=$edge4_data_mac,nw_dst=$edge5_data_ip,actions=LOCAL
sudo ovs-ofctl add-flow br1 ip,priority=100,in_port=2,dl_src=$edge6_data_mac,nw_dst=$edge5_data_ip,actions=LOCAL
...
sudo ovs-ofctl add-flow br0 ip,priority=100,in_port=1,dl_src=$edge2_control_mac,nw_dst=$edge6_control_ip,actions="resubmit(,4)" #edge6 mac change
sudo ovs-ofctl add-flow br0 ip,priority=100,in_port=1,dl_src=$edge6_control_mac,nw_dst=$superedge_control_ip,actions="resubmit(,2)" #edge2 mac change
sudo ovs-ofctl add-flow br0 ip,priority=100,in_port=1,dl_src=$edge4_control_mac,nw_dst=$edge6_control_ip,actions="resubmit(,4)" #edge6 mac change
sudo ovs-ofctl add-flow br0 ip,priority=100,in_port=1,dl_src=$edge6_control_mac,nw_dst=$superedge_control_ip,actions="resubmit(,3)" #edge4 mac change

#Relay for edge6 the incoming data traffic to  @edge5 

sudo ovs-ofctl add-flow br1 ip,priority=100,in_port=2,dl_src=$edge2_data_mac,nw_dst=$edge6_data_ip,actions="resubmit(,4)" #edge6 mac change
sudo ovs-ofctl add-flow br1 ip,priority=100,in_port=2,dl_src=$edge6_data_mac,nw_dst=$superedge_data_ip,actions="resubmit(,2)" #edge2 mac change
sudo ovs-ofctl add-flow br1 ip,priority=100,in_port=2,dl_src=$edge4_data_mac,nw_dst=$edge6_data_ip,actions="resubmit(,4)" #edge6 mac change
sudo ovs-ofctl add-flow br1 ip,priority=100,in_port=2,dl_src=$edge6_data_mac,nw_dst=$superedge_data_ip,actions="resubmit(,3)" #edge4 mac change

#Relay for edge1 the incoming  control traffic to @edge5

sudo ovs-ofctl add-flow br0 ip,priority=100,in_port=1,dl_src=$edge2_control_mac,nw_dst=$edge1_control_ip,actions="resubmit(,3)" #edge4 mac change
...
#Relay for edge1 the incoming data traffic to @edge5 

sudo ovs-ofctl add-flow br1 ip,priority=100,in_port=2,dl_src=$edge2_data_mac,nw_dst=$edge1_data_ip,actions="resubmit(,3)" #edge4 mac change
...
#Relay for edge2 the incoming  control traffic to @edge5

sudo ovs-ofctl add-flow br0 ip,priority=100,in_port=1,dl_src=$edge4_control_mac,nw_dst=$edge2_control_ip,actions="resubmit(,2)" #edge2 mac change
sudo ovs-ofctl add-flow br0 ip,priority=100,in_port=1,dl_src=$edge2_control_mac,nw_dst=$superedge_control_ip,actions="resubmit(,3)" #edge4 mac change
sudo ovs-ofctl add-flow br0 ip,priority=100,in_port=1,dl_src=$edge5_control_mac,nw_dst=$superedge_control_ip,actions="resubmit(,4)" #edge6 mac change
sudo ovs-ofctl add-flow br0 ip,priority=100,in_port=1,dl_src=$edge6_control_mac,nw_dst=$edge2_control_ip,actions="resubmit(,2)" #edge2 mac change

#Relay for edge2 the incoming data traffic to @edge5 

sudo ovs-ofctl add-flow br1 ip,priority=100,in_port=2,dl_src=$edge4_data_mac,nw_dst=$edge2_data_ip,actions="resubmit(,2)" #edge2 mac change
sudo ovs-ofctl add-flow br1 ip,priority=100,in_port=2,dl_src=$edge2_data_mac,nw_dst=$superedge_data_ip,actions="resubmit(,3)" #edge4 mac change
sudo ovs-ofctl add-flow br1 ip,priority=100,in_port=2,dl_src=$edge5_data_mac,nw_dst=$superedge_data_ip,actions="resubmit(,4)" #edge6 mac change
sudo ovs-ofctl add-flow br1 ip,priority=100,in_port=2,dl_src=$edge6_data_mac,nw_dst=$edge2_data_ip,actions="resubmit(,2)" #edge2 mac change

#Relay for edge3 the incoming  control traffic to @edge5

sudo ovs-ofctl add-flow br0 ip,priority=100,in_port=1,dl_src=$edge2_control_mac,nw_dst=$edge3_control_ip,actions="resubmit(,4)" #edge6 mac change
sudo ovs-ofctl add-flow br0 ip,priority=100,in_port=1,dl_src=$edge6_control_mac,nw_dst=$superedge_control_ip,actions="resubmit(,2)" #edge2 mac change


#Relay for edge3 the incoming data traffic to @edge5 

sudo ovs-ofctl add-flow br1 ip,priority=100,in_port=2,dl_src=$edge2_data_mac,nw_dst=$edge3_data_ip,actions="resubmit(,4)" #edge6 mac change
sudo ovs-ofctl add-flow br1 ip,priority=100,in_port=2,dl_src=$edge6_data_mac,nw_dst=$superedge_data_ip,actions="resubmit(,2)" #edge2 mac change

#Relay for edge4 the incoming  control traffic to @edge5
...
#Table 4 is to rewrite the destination MAC address into edge4 MAC address @ data plane
