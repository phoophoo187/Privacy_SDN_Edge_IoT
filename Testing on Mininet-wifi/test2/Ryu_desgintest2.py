...
#Relay To Gateway 1
	    ...
	    match = parser.OFPMatch(in_port=1, eth_type=0x0806,arp_tpa=gw1ip)
	    ...
	    match = parser.OFPMatch(in_port=1, eth_type=0x0800,ipv4_dst=gw1ip)
            actions = [parser.OFPActionSetField(eth_src=r1mac),parser.OFPActionSetField(eth_dst=gw1mac),parser.OFPActionOutput(datapath.ofproto.OFPP_IN_PORT)]
            self.add_flow(datapath,0,160,match,actions,0)

##Relay to Raspi 1
	    match = parser.OFPMatch(in_port=1, eth_type=0x0800,ipv4_dst=r2ip)
	    ...
	    match = parser.OFPMatch(in_port=1, eth_type=0x0806,arp_tpa=r2ip)
            actions = [parser.OFPActionSetField(eth_src=r1mac),parser.OFPActionSetField(eth_dst=r2mac),parser.OFPActionOutput(datapath.ofproto.OFPP_IN_PORT)]
            self.add_flow(datapath,0,160,match,actions,0)

###Relay To Raspi3
	    match = parser.OFPMatch(in_port=1, eth_type=0x0806,arp_tpa=r3ip)
            actions = [parser.OFPActionSetField(eth_src=r1mac),parser.OFPActionSetField(eth_dst=r2mac),parser.OFPActionOutput(datapath.ofproto.OFPP_IN_PORT)]
            self.add_flow(datapath,0,160,match,actions,0)

	    match = parser.OFPMatch(in_port=1, eth_type=0x0800,ipv4_dst=r3ip)
            actions = [parser.OFPActionSetField(eth_src=r1mac),parser.OFPActionSetField(eth_dst=r2mac),parser.OFPActionOutput(datapath.ofproto.OFPP_IN_PORT)]
            self.add_flow(datapath,0,160,match,actions,0)

##Relay To Gateway2
	    match = parser.OFPMatch(in_port=1, eth_type=0x0806,arp_tpa=gw2ip)
            actions = [parser.OFPActionSetField(eth_src=r1mac),parser.OFPActionSetField(eth_dst=r2mac),parser.OFPActionOutput(datapath.ofproto.OFPP_IN_PORT)]
            self.add_flow(datapath,0,160,match,actions,0)

	    match = parser.OFPMatch(in_port=1, eth_type=0x0800,ipv4_dst=gw2ip)
            actions = [parser.OFPActionSetField(eth_src=r1mac),parser.OFPActionSetField(eth_dst=r2mac),parser.OFPActionOutput(datapath.ofproto.OFPP_IN_PORT)]
            self.add_flow(datapath,0,160,match,actions,0)
...
#######Relay To Raspi1
	    match = parser.OFPMatch(in_port=1, eth_type=0x0806,arp_tpa=r1ip)
	    ...
	    match = parser.OFPMatch(in_port=1, eth_type=0x0800,ipv4_dst=r1ip)
            actions = [parser.OFPActionSetField(eth_src=r2mac),parser.OFPActionSetField(eth_dst=r1mac),parser.OFPActionOutput(datapath.ofproto.OFPP_IN_PORT)]
            self.add_flow(datapath,0,150,match,actions,0)
...
#####Raspi4
	    match = parser.OFPMatch(in_port=1,eth_type=0x0806,arp_tpa=r4ip)
	    ...
	    match = parser.OFPMatch(in_port=1,eth_type=0x0800,ipv4_dst=r4ip)
            actions = [parser.OFPActionSetField(eth_src=gw1mac),parser.OFPActionSetField(eth_dst=r4mac),parser.OFPActionOutput(datapath.ofproto.OFPP_IN_PORT)]
            self.add_flow(datapath,0,160,match,actions,0)

#######Raspi5
	    match = parser.OFPMatch(in_port=1,eth_type=0x0806,arp_tpa=r5ip)
            actions = [parser.OFPActionSetField(eth_src=gw1mac),parser.OFPActionSetField(eth_dst=r4mac),parser.OFPActionOutput(datapath.ofproto.OFPP_IN_PORT)]
            self.add_flow(datapath,0,160,match,actions,0)

	    match = parser.OFPMatch(in_port=1,eth_type=0x0800,ipv4_dst=r5ip)
            actions = [parser.OFPActionSetField(eth_src=gw1mac),parser.OFPActionSetField(eth_dst=r4mac),parser.OFPActionOutput(datapath.ofproto.OFPP_IN_PORT)]
            self.add_flow(datapath,0,160,match,actions,0)

#####Raspi6
	    match = parser.OFPMatch(in_port=1,eth_type=0x0806,arp_tpa=r6ip)
            actions = [parser.OFPActionSetField(eth_src=gw1mac),parser.OFPActionSetField(eth_dst=r4mac),parser.OFPActionOutput(datapath.ofproto.OFPP_IN_PORT)]
            self.add_flow(datapath,0,160,match,actions,0)

	    match = parser.OFPMatch(in_port=1,eth_type=0x0800,ipv4_dst=r6ip)
            actions = [parser.OFPActionSetField(eth_src=gw1mac),parser.OFPActionSetField(eth_dst=r4mac),parser.OFPActionOutput(datapath.ofproto.OFPP_IN_PORT)]
            self.add_flow(datapath,0,160,match,actions,0)
