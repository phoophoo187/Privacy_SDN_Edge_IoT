...
class node_failure (app_manager.RyuApp):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.edge_list = []
        self.weight_edge_list = [2, 2, 3, 2, 1, 2, 2, 3, 1, 2]
        self.concave_edge_list = [1, 3, 3, 1, 4, 3, 1, 1, 2, 3]        
        self.CPU_edge_list = [2, 1, 2, 2, 2, 3, 4, 4, 1, 1]  
    ...
    def edge_monitor_routing(self, source, dest, L):                       
        ...
        option1_row = ['1', source, dest, optimal_cost, optimal_path]
        
        self.G = nx.Graph()
        self.G.add_edges_from(self.edge_list) 
        self.G = self.add_multi_link_attributes(self.weight_edge_list, self.concave_edge_list, self.CPU_edge_list)                                 
        optimal_cost, optimal_path = self.Option2_routing(source, dest, L) 
        option2_row = ['2', source, dest, optimal_cost, optimal_path]

        self.G = nx.Graph()
        self.G.add_edges_from(self.edge_list) 
        self.G = self.add_multi_link_attributes(self.weight_edge_list, self.concave_edge_list, self.CPU_edge_list)                    
        optimal_cost, optimal_path = self.Option3_routing(source, dest, L)
        option3_row = ['3', source, dest, optimal_cost, optimal_path]
        ...
        while True:   
            if len(self.datapaths.keys()) == 7:       
                ...
                for i in range(len(self.edge_list)):
                    network_table.add_row([self.edge_list[i], self.weight_edge_list[i], self.concave_edge_list[i], self.CPU_edge_list[i]])
                print(network_table)
                ...
                routing_table = PrettyTable(['Option', 'source', 'dest', 'path cost', 'optimal path'])                       
