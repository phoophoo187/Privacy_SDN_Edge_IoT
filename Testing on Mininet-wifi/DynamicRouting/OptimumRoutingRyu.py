...
API_ENDPOINT = "http://localhost:9696/updateNetworkInfo"
...
class node_failure (app_manager.RyuApp):
    ...
    def getOptionText(self, option):
        if option == 1: return 'Option 1 - Discard one of the concave matrics'
        elif option == 2: return 'Option 2 - Combine two concave metrics with weighted sum'
        elif option == 3: return 'Option 3 - Combine two concave metrics with concave function'
    
    def edge_monitor_routing(self, source, dest, L, option = None):                                                  
        self.G = nx.DiGraph()                                
    ...
    def convertDictToArr(self, data):
        rData = []
        for (u, v) in self.G.edges():
            rData.append({
                'link': u + '-' + v,
    ...
    def getBackendJson(self, src, dest, L, option, optimal_path):
        dt = datetime.datetime.now()
        json = {
            'source': src,
            'destination': dest,
            'constraint': L,
            'option': self.getOptionText(option),
            'optimalPath': optimal_path,
            'cpuInfo': self.CPU_list,
            'weightInfo': self.convertDictToArr(self.weight_edge_list),
            'concaveInfo': self.convertDictToArr(self.concave_edge_list),
            'dateTime': dt.strftime("%x %X")
        }
        return json
    ...
    def _monitor(self):     
        while True:   
            if len(self.datapaths.keys()) == 7:       
                ...
                self.G.add_edges_from(self.edge_list)                 
                self.add_multi_link_attributes()      
                self.logger.info(self.getOptionText(option))                  
                ...
                optimal_path = self.edge_monitor_routing(src, dest, L, option)                  
                routing_table.add_row([option, src, dest, optimal_path])                
                
                optimal_path = self.edge_monitor_routing('edge2', dest, L, option)                  
                routing_table.add_row([option, 'edge2', dest, optimal_path])                
                
                optimal_path = self.edge_monitor_routing('edge3', dest, L, option)                  
                routing_table.add_row([option, 'edge3', dest, optimal_path])                
                
                optimal_path = self.edge_monitor_routing('edge4', dest, L, option)                  
                routing_table.add_row([option, 'edge4', dest, optimal_path])                
                
                optimal_path = self.edge_monitor_routing('edge5', dest, L, option)                  
                routing_table.add_row([option, 'edge5', dest, optimal_path])                
                
                optimal_path = self.edge_monitor_routing('edge6', dest, L, option)                  
                routing_table.add_row([option, 'edge6', dest, optimal_path])                
                
                print(routing_table)
                                
                #r = requests.post(url = API_ENDPOINT, json = self.getBackendJson(src, dest, L, option, optimal_path))
                #self.logger.info("Response data from node API backend : %s", r.text)
